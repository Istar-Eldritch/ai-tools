...
describe("Git file tracking utilities", () => {
	...
	describe("backward compatibility (Phase 5)", () => {
		let testDir: string;

		beforeEach(async () => {
			testDir = await mkdtemp(join(tmpdir(), "spec-pipeline-git-compat-"));
			await execGit(testDir, ["init"]);
			await execGit(testDir, ["config", "user.email", "test@example.com"]);
			await execGit(testDir, ["config", "user.name", "Test User"]);
		...
		it("createCheckpointAndSave creates checkpoints for old pipelines without useAgentCommits", async () => {
			...
			expect(state.checkpoints[0]).toBeTruthy();
		});

		it("createAgentCommit skips commits for old pipelines without useAgentCommits", async () => {
			const { createAgentCommit } = await import("./git.ts");
		...
		it("createAgentCommit processes new pipelines with useAgentCommits", async () => {
			const { createAgentCommit } = await import("./git.ts");
			...
			// Don't create any file modifications, so createAgentCommit should skip
			// due to no files modified (but it won't skip due to useAgentCommits check)
			const result = await createAgentCommit(
				testDir,
				state,
				{
			...
			// and skipped due to no modified files (not due to useAgentCommits)
			expect(result.success).toBe(true);
			expect(result.commitHash).toBeUndefined();
		});
	});
